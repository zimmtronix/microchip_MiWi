...
/************************ DATA TYPE *******************************/
typedef union _MIWI_STATE_MACHINE
{
    uint16_t Val;
    struct {
        ...
        uint8_t MiWiAckInProgress      :1;
        uint8_t saveConnection         :1;
...
typedef struct 
{
    ...
    uint8_t        PayLoadSize;                // the indirect message pay load size
        ...
        uint8_t    PayLoad[110];
    #else
        uint8_t    PayLoad[TX_BUFFER_SIZE+MIWI_HEADER_LEN];    // the indirect message pay load
    #endif
} INDIRECT_MESSAGE;
