...
volatile uint8_t cnt;
    ...
    bool TxPacket(INPUT uint8_t TxPacketLen, INPUT bool CCA)
    {
        ...
        #if defined(ENABLE_ACKENABLE_ACKENABLE_ACKENABLE_ACK) && defined(ENABLE_RETRANSMISSION)
    ...
    if( PHY_IRQ1 && PHY_IRQ1_En )
    {
        if(RF_Mode == RF_RECEIVER)
        {
            ...
            for(BankIndex = 0; BankIndex < BANK_SIZE; BankIndex++)
            {
             ...
             // may be an acknowledgent (If packetlen = 2, packet information =
            if( PacketLen == 2 )
            {
                bAck = true;
            }
            ...
            while(counter >= 1)
            {
                ...
                if(1)
                {
                    ...
                    if( RxPacketPtr >= PacketLen )
                    {
                        ...
                        // send ack / check ack
                        #if defined(ENABLE_ACK)
                            if( ( RxPacket[BankIndex].Payload[0] & PACKET_TYPE_MASK ) == PACKET_TYPE_ACK )  // acknowledgment
                            {
...
RETURN_HERE:     
        PHY_IRQ1 = 0;  // Clear Interrupt Flag
