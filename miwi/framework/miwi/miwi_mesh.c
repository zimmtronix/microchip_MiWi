...
uint8_t            TxData;
    ...
    uint8_t TxBuffer[110];
#else
    uint8_t TxBuffer[TX_BUFFER_SIZE+MIWI_HEADER_LEN];
#endif
...
uint8_t MiWiSeqNum;
    ...
    bool MiWiAckRequired = false;
        ...
        #endif
    #endif
    uint8_t RoutingTable[8];
    uint8_t RouterFailures[8];
    uint8_t knownCoordinators;
    uint8_t role;
#endif
...
void SendBeacon(void);
    ...
    uint8_t getConnMode(){return ConnMode;}
    uint8_t getIsValid(uint8_t iii){return ConnectionTable[iii].status.bits.isValid;}
    uint8_t getlongAddressValid(uint8_t iii){return ConnectionTable[iii].status.bits.longAddressValid;}
    uint8_t getshortAddressValid(uint8_t iii){return ConnectionTable[iii].status.bits.shortAddressValid;}
    uint8_t getConnectionAddress(uint8_t map,uint8_t id){return ConnectionTable[map].Address[id];}
    uint8_t getRXOnWhenIdle(uint8_t iii){return ConnectionTable[iii].status.bits.RXOnWhenIdle;}
    uint8_t getdirectConnection(uint8_t iii){return ConnectionTable[iii].status.bits.directConnection;}
    uint8_t getPeerInfo(uint8_t iii,uint8_t jjj){return ConnectionTable[iii].PeerInfo[jjj];}
    uint8_t getAltAddress(uint8_t iii,uint8_t jjj){return ConnectionTable[iii].AltAddress.v[jjj];}
    uint8_t getPANID(uint8_t iii,uint8_t jjj){return ConnectionTable[iii].PANID.v[jjj];}
                                            ...
                                            if(ItsMe == false)
                                            {
                                                ...
                                                if(handle!=0xFF)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        //CONSOLE_PutString((char*)"found long address but short address not valid\r\n");
                                                ...
                                                else
                                                {
                                                    ...
                                                    //CONSOLE_PutString((char*)"couldn't find the device");
                                                    TxData = 0;
                                                }
                                            }
                                            else
                                    ...
                                    case OPEN_SOCKET_REQUEST:
                                        {
                                            ...
                                            else
                                            {
                                                    ...
                                                    (MACRxPacket.Payload[MIWI_HEADER_LEN-2] == openSocketInfo.ShortAddress1.v[1]))
                                                {
                                                    ...
                                                    //CONSOLE_PutString((char*)"got request from same guy twice\r\n");
                                                }
                                                ...
                                                {
                                                    //CONSOLE_PutString((char*)"sending out response\r\n");
            ...
            case PACKET_TYPE_BEACON:
                {
                    ...
                    #ifdef NWK_ROLE_COORDINATOR
                    {
                        ...
                        #if defined(IEEE_802_15_4)
                            uint8_t coordinatorNumber = MACRxPacket.SourceAddress[1];
                        #else
                            uint8_t coordinatorNumber = MACRxPacket.Payload[3];
                        #endif
                        uint8_t mask = 1<<(coordinatorNumber);
...
START_ASSOCIATION_RESPONSE:                          
                    ...
                    case MAC_COMMAND_ASSOCIATION_RESPONSE:
                        {
                            if(MACRxPacket.Payload[3] == ASSOCIATION_SUCCESSFUL)
                            {
                                ...
                                myShortAddress.v[1] = MACRxPacket.Payload[2];
                                    ...
                                    {
                                        //CONSOLE_PutString((char*)"I am a coordinator\r\n");
                                    ...
                                    {
                                        //CONSOLE_PutString((char*)"I am an end device\r\n");
    ...
    if(openSocketInfo.status.bits.requestIsOpen)
    {
        if(openSocketInfo.status.bits.matchFound)
        {
                ...
                //CONSOLE_PutString((char*)"sending out second response\r\n");
    ...
    bool RouteMessage(API_UINT16_UNION PANID, API_UINT16_UNION ShortAddress, bool SecEn)
    {
        uint8_t parentNode = (ShortAddress.v[1] & 0x07);
        uint8_t i;
...
void DumpConnection(uint8_t index)
{
    ...
    if( index == 0xFF )
    {
        ...
        Printf("  Short Addr: ");
        CONSOLE_PrintHex(myShortAddress.v[1]);
        CONSOLE_PrintHex(myShortAddress.v[0]);
        Printf("  PANID: ");
    ...
    if( index == 0xFF )
    {
        for(i=0;i<CONNECTION_SIZE;i++)
        {
            if(ConnectionTable[i].status.bits.isValid)
            {
                ...
                {
                    //this is a neighbor
                    CONSOLE_PrintHex(ConnectionTable[i].PANID.v[1]);
                    CONSOLE_PrintHex(ConnectionTable[i].PANID.v[0]);
                    CONSOLE_Put(' ');
                    CONSOLE_Put(' ');
                    CONSOLE_PrintHex(ConnectionTable[i].AltAddress.v[1]);
                    CONSOLE_PrintHex(ConnectionTable[i].AltAddress.v[0]);
                    CONSOLE_Put(' ');
                }
                else
                {
    ...
    else
    {
        if(ConnectionTable[index].status.bits.isValid)
        {
            ...
            {
                //this is a neighbor
                CONSOLE_PrintHex(ConnectionTable[index].PANID.v[1]);
                CONSOLE_PrintHex(ConnectionTable[index].PANID.v[0]);
                CONSOLE_Put(' ');
                CONSOLE_Put(' ');
                CONSOLE_PrintHex(ConnectionTable[index].AltAddress.v[1]);
                CONSOLE_PrintHex(ConnectionTable[index].AltAddress.v[0]);
                CONSOLE_Put(' ');
            }
            else
            {
            ...
            if(ConnectionTable[index].status.bits.longAddressValid)
            {
                ...
                for(j = 0; j < MY_ADDRESS_LENGTH; j++)
                {
                    CONSOLE_PrintHex(ConnectionTable[index].Address[MY_ADDRESS_LENGTH-1-j]);
                }
            }
                ...
                for(j = 0; j < ADDITIONAL_NODE_ID_SIZE; j++)
                {
                    CONSOLE_PrintHex(ConnectionTable[index].PeerInfo[j]);
                }
            #endif
    ...
    #ifdef NWK_ROLE_COORDINATOR
        Printf("\r\nKnown Coordinators: ");
        CONSOLE_PrintHex(knownCoordinators);
        //CONSOLE_PutString((ROM char*)"\r\nRouting Table\r\ncoord nextHop\r\n");
        for(i=0;i<8;i++)
        {
            CONSOLE_PrintHex(i);
            CONSOLE_Put(' ');
            ...
            CONSOLE_Put(' ');
            CONSOLE_Put(' ');
            CONSOLE_PrintHex(RoutingTable[i]);
            CONSOLE_Put('\r');
            CONSOLE_Put('\n');
        }
    #endif
...
GetOutOfLoop:
        ...
        bool MiApp_InitChannelHopping( INPUT uint32_t ChannelMap)
        {
            ...
            Printf("\r\nHopping to Channel ");
            CONSOLE_PrintDec(optimalChannel);
            Printf("\r\n");
